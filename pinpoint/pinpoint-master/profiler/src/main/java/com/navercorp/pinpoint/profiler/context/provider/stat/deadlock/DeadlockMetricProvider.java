...
import com.navercorp.pinpoint.profiler.monitor.metric.deadlock.DefaultDeadlockMetric;

/**
 * @author Taejin Koo
 */
public class DeadlockMetricProvider implements Provider<DeadlockMetric> {

    private final DeadlockThreadRegistry deadlockThreadRegistry;

    @Inject
    public DeadlockMetricProvider(DeadlockThreadRegistry deadlockThreadRegistry) {
        this.deadlockThreadRegistry = deadlockThreadRegistry;
    }

    @Override
    public DeadlockMetric get() {
        return new DefaultDeadlockMetric(deadlockThreadRegistry);
    }
