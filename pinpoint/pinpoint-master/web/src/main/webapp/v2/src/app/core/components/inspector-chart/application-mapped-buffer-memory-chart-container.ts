...
export class ApplicationMappedBufferMemoryChartContainer implements IInspectorChartContainer {
    ...
    makeChartData({charts}: IInspectorChartData): PrimitiveArray[] {
        this.minAgentIdList = makeYData(charts.y['MAPPED_MEMORY_USED'], 1) as string[];
        this.maxAgentIdList = makeYData(charts.y['MAPPED_MEMORY_USED'], 3) as string[];

        return [
            ['x', ...makeXData(charts.x)],
            ['min', ...makeYData(charts.y['MAPPED_MEMORY_USED'], 0)],
            ['avg', ...makeYData(charts.y['MAPPED_MEMORY_USED'], 4)],
            ['max', ...makeYData(charts.y['MAPPED_MEMORY_USED'], 2)],
        ];
    }
