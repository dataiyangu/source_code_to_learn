...
class CglibAopProxy implements AopProxy, Serializable {
	...
	//注释版本的
	/*关于代理逻辑的织入，其实现主体还是通过Enhancer来实现，即通过需要织入的Advisor列表，生成Callback对象，
	并将其设置到Enhancer对象中，最后通过Enhancer生成目标对象。如下是AopProxy.getProxy()方法的源码：*/
	public Object getProxy(@Nullable ClassLoader classLoader) {
		if (logger.isDebugEnabled()) {
	...
	//注释的版本
	/*可以看到，这里的AopProxy.getProxy()方法就是生成代理对象的主干逻辑。上面的逻辑中主要有两个部分需要重点讲解：
			①如果获取Callback数组；
			②CallbackFilter的作用。关于第一点，我们后面会进行重点讲解，至于第二点，这里我们需要理解的就是
	CallbackFilter.accept()方法接收一个Method类型的参数，该参数也即当前要生成的代理逻辑的方法，
	这里的accept()方法将返回目标当前要织入代理逻辑的方法所需要使用的切面逻辑，也即Callback对象在Callback数组中的下标。
	关于CallbackFilter的使用原理，
	读者可以阅读实战CGLib系列之proxy篇(二)：回调过滤CallbackFilter这篇文章。下面我们继续阅读getCallbacks()的源码：*/

	/*这里的getCallbacks()方法主要做了三件事：
			①获取目标对象的动态调用链；
			②判断是否设置了exposeProxy属性，如果设置了，则生成一个可以暴露代理对象的Callback对象，
	否则生成一个不做任何处理直接调用目标对象的Callback对象；③判断目标对象是否是静态的，并且当前的切面逻辑是否是固定的，
	如果是，则将目标对象和调用链进行缓存，以便后续直接调用。这里需要说明的一个点在于第三点，因为在判断目标对象为静态对象，
	并且调用链是固定的时候，会将目标对象和调用链进行缓存，并且封装到指定的Callback对象中。
	这里读者可能会疑问为什么动态调用链和静态调用链都进行了缓存，这和前面讲解的CallbackFilter是息息相关的，
	因为上述代码最后使用fixedInterceptorOffset记录了当前静态调用链在数组中存储的位置，我们前面也讲了，Enhancer可以通过
	CallbackFilter返回的整数值来动态的指定从当前对象Callback数组中的第几个环绕逻辑开始织入，
	这里就会使用到fixedInterceptorOffset。从上述代码中可以看出，用户自定义的调用链是在DynamicAdvisedInterceptor中生成的
	（关于静态调用链的生成实际上是同样的逻辑，只不过静态调用链会被缓存），这里我们看看DynamicAdvisedInterceptor的实现源码：*/
	private Callback[] getCallbacks(Class<?> rootClass) throws Exception {
		boolean exposeProxy = this.advised.isExposeProxy();
		boolean isFrozen = this.advised.isFrozen();
		boolean isStatic = this.advised.getTargetSource().isStatic();
	...
	private static class DynamicAdvisedInterceptor implements MethodInterceptor, Serializable {
		...
		// }
		//注释的版本
		/*这里intercept()方法里主要逻辑有两点：
				①为目标对象生成切面逻辑调用链；
				②通过切面逻辑对目标对象进行环绕，并且进行调用。关于这两点，我们都会进行讲解，这里我们首先看看Cglib是如何生成调用链的，
		如下是getInterceptorsAndDynamicInterceptionAdvice()方法最终调用的源码，中间略过了部分比较简单的调用：*/
